...
logging.basicConfig(filename="message.log",format='%(asctime)s: %(levelname)s: %(message)s',level=logging.INFO)
...
_, X_test, _, y_test = train_test_split( df['text_input'].to_numpy(dtype=str), df['label_idx'].to_numpy(dtype=int),
    test_size=0.2, random_state=42, stratify=df['label_idx'].to_numpy(dtype=int))
...
def report(name: str, elapsed_mean: float, elapsed_std: float, n: int):
    ...
    return { 'model': name, 'total_s': round(elapsed_mean, 3), 'std_s': round(elapsed_std, 3),
        'ms_per_sample': round(ms_per_sample, 3), 'samples_per_sec': round(n / elapsed_mean, 1)
    }
...
# =============================================================================
# 4. BERT/FAKEBERT TOKENISATION PREPARATION
# =============================================================================
...
bert_mean, bert_std = measure(lambda: run_inference(bert_model), args.runs)
bert_result = report('BERT-base', bert_mean, bert_std, len(X_test))
del bert_model


# =============================================================================
# 6. FAKEBERT DEFINITION & INFERENCE
# =============================================================================
...
fakebert_model.load_state_dict(
    torch.load('outputs/fakebert_finetuned/fakebert_weights.pt',
               map_location=DEVICE, weights_only=True)
)

fakebert_mean, fakebert_std = measure(lambda: run_inference(fakebert_model), args.runs)
