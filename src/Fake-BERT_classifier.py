...
def main() -> None:
    ...
    run = wandb.init(
        ...
        name=f"fakebert-{args.dataset}-seed{args.seed}",
        tags=["fakebert", args.dataset, f"seed{args.seed}"],
        config={
            "model": f"{MODEL_NAME}+CNN",
    ...
    df["text_input"] = df["text_input"].astype(str)
    ...
    plt.tight_layout()
    curves_path = "outputs/fakebert_training_curves.png"
    plt.savefig(curves_path, dpi=150, bbox_inches="tight")
    wandb.log({"charts/training_curves": wandb.Image(curves_path)})
    logging.info(f"Saved: {curves_path}")
    if args.plot:
        plt.show()
    plt.close()
    ...
    plt.tight_layout()
    cm_path = "outputs/fakebert_confusion_matrix.png"
